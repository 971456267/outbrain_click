...
from sklearn.metrics import roc_auc_score,precision_score,accuracy_score,recall_score,classification_report
def get_max():
    """
...
if __name__ == "__main__":  
    ...
    for i in range(1,7):
        train_info = read_feature("./processed_data/split_data/train_{0}.csv".format(i))
        ...
        train_data_all = [train_info[3],train_info[4],train_info[5],\
                      ...
                      train_info[15],train_info[16],train_info[17],train_info[18],train_info[19]]#,train_info[20]]
        del train_info
        gc.collect()  
        save_model_file = "./save_model/model_weight.h5"
        if os.path.exists(save_model_file):
            model.load_weights(save_model_file)
        model.fit(train_data_all,y_train,batch_size = 2048,epochs = 5)
    ...
    valid_info = pd.read_csv("./processed_data/split_data/valid_4.csv")
    valid_info = split_feature_into_multi_input(valid_info) 
    y_valid = valid_info[0]  
    ...
    valid_data_all = [valid_info[3],valid_info[4],valid_info[5],\
                      ...
                      valid_info[15],valid_info[16],valid_info[17],valid_info[18],valid_info[19]]#,valid_info[20]]
